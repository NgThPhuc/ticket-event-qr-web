...
const EditEvent = () => {
    ...
    const fetchOrganizations = async () => {
      ...
      try {
        ...
        const orgList = allowedOrgs.map(org => {
          const orgData = org.organization || { id: org.organization_id || org.id, name: org.organization?.name || 'Unknown' };
      ...
      try {
        ...
        if (!isPlatformAdmin) {
          ...
          try {
            ...
            if (!userRole || (userRole !== 'ORGANIZER_ADMIN' && userRole !== 'EVENT_MANAGER')) {
              setAlert({
                type: 'error',
        ...
        setAlert({
          type: 'error',
          message: err.message || t('event.fetchDetailError') || 'Không thể tải thông tin sự kiện',
        });
        if (err.status === 404) {
          setTimeout(() => {
            navigate('/events-management');
  ...
  const validate = () => {
    ...
    if (formData.attendance_mode === 'OFFLINE' || formData.attendance_mode === 'HYBRID') {
    ...
    if (formData.attendance_mode === 'ONLINE' || formData.attendance_mode === 'HYBRID') {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.organization_id) payload.organization_id = formData.organization_id;
      ...
      if (formData.attendance_mode === 'ONLINE' || formData.attendance_mode === 'HYBRID') {
        ...
        if (formData.stream_platform?.trim()) payload.stream_platform = formData.stream_platform.trim();
      }
      
      if (formData.capacity_total) payload.capacity_total = parseInt(formData.capacity_total);
      ...
      <DashboardLayout>
        <div className="container mx-auto px-4 py-8">
          <Alert variant="destructive">
            <AlertDescription>{alert.message || 'Không tìm thấy sự kiện'}</AlertDescription>
          </Alert>
        </div>
      </DashboardLayout>
    );
  ...
  // Use same form structure as CreateEvent
  return (
    <DashboardLayout>
      <div className="container mx-auto px-4 py-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => handleSelectChange('attendance_mode', value)}
              >
                <SelectTrigger className={errors.attendance_mode ? 'border-red-500' : ''}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
              ...
              {errors.attendance_mode && (
                <p className="mt-1 text-sm text-red-600">{errors.attendance_mode}</p>
              )}
            </div>
            ...
            {(formData.attendance_mode === 'OFFLINE' || formData.attendance_mode === 'HYBRID') && (
              <div className="space-y-4 border-t pt-4">
                <h3 className="font-semibold text-lg">Thông tin địa điểm</h3>
            ...
            {(formData.attendance_mode === 'ONLINE' || formData.attendance_mode === 'HYBRID') && (
              <div className="space-y-4 border-t pt-4">
                <h3 className="font-semibold text-lg">Thông tin trực tuyến</h3>
            ...
            <div className="grid grid-cols-2 gap-4 border-t pt-4">
              <div>
                <Label htmlFor="capacity_total">{t('event.capacityTotal')}</Label>
                <Input
                  id="capacity_total"
                  type="number"
                  name="capacity_total"
            ...
            <div className="flex gap-4 pt-4">
              ...
              <Button type="submit" disabled={submitting} className="flex-1">
                {submitting ? t('common.loading') : t('event.updateButton')}
              </Button>
            </div>
          </form>
        </div>
