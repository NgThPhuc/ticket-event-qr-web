...
import { AlertCircle, ArrowLeft, Calendar, CheckCircle2, Clock, CreditCard, MapPin, Ticket, Timer, XCircle } from "lucide-react";
...
const OrderCountdown = ({ expiresAt, onExpired }) => {
    ...
    return (
        <div className={`flex items-center gap-2 px-4 py-3 rounded-lg font-medium transition-all ${isWarning
...
const CustomerOrderDetail = () => {
    ...
    const handlePayNow = async () => {
        try {
            ...
            const response = await initiatePayment(orderId, returnUrl, cancelUrl, 'PAYOS');
            ...
            toast.error(err.message || t('payment.errors.initFailed'));
        } finally {
    ...
    const getStatusBadge = (status) => {
        const config = {
            PENDING: { variant: 'secondary', icon: Clock, className: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400' },
            CONFIRMED: { variant: 'default', icon: CheckCircle2, className: 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' },
            CANCELLED: { variant: 'destructive', icon: XCircle, className: 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400' },
            COMPLETED: { variant: 'success', icon: CheckCircle2, className: 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400' }
        };
    ...
    const getPaymentStatusBadge = (status) => {
        const config = {
            UNPAID: { className: 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400' },
            PAID: { className: 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' },
            REFUNDED: { className: 'bg-gray-100 text-gray-800 dark:bg-gray-900/30 dark:text-gray-400' }
        };
        const { className } = config[status] || config.UNPAID;
        return <Badge className={className}>{t(`order.paymentStatus.${status}`)}</Badge>;
    };

    const canCancel = order && order.status === 'PENDING' && order.payment_status === 'UNPAID';
    const canPay = order && order.status === 'PENDING' && order.payment_status === 'UNPAID';
                                ...
                                <CardContent className="space-y-4">
                                    ...
                                    {order.event.slug && (
                                        <Button
                                            variant="outline"
                                            size="sm"
                                            onClick={() => navigate(`/e/${order.event.slug}`)}
                                        >
                        ...
                        {order.tickets && order.tickets.length > 0 && (
                            <Card>
                                <CardHeader>
                                    <CardTitle className="flex items-center gap-2">
                                        <Ticket className="w-5 h-5" />
                                        {t('order.tickets')} ({order.tickets.length})
                                    </CardTitle>
                                </CardHeader>
                                <CardContent>
                        ...
                        <div className="flex flex-col sm:flex-row gap-3">
                            {canPay && (
                                ...
                                >
                                    ...
                                    {processingPayment ? t('common.loading') : t('order.payNow')}
                                </Button>
                            )}
                            {canCancel && (
                                <Button
                            ...
                            <CardContent className="space-y-4">
                                ...
                                <div className="space-y-2">
                                    <div className="flex justify-between text-sm">
                                        ...
                                        <span>{order.quantity} {t('order.ticketsUnit')}</span>
                                    </div>
