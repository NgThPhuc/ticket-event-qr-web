...
import { Activity, Clock, Percent, TrendingUp, UserCheck, Users } from 'lucide-react';
...
export function CheckInStatsCard({ stats, loading = false, compact = false }) {
    ...
    return (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            {/* Total Scans */}
            {stats.total_scans !== undefined && (
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium">
                            {t('checkin.stats.totalScans', 'Tổng lượt quét')}
                        </CardTitle>
                    ...
                    <CardContent>
                        ...
                        <p className="text-xs text-muted-foreground">
                            <span className="text-red-500">{stats.failed_scans || 0}</span> {t('checkin.stats.failed', 'thất bại')}
                        </p>
                    </CardContent>
                </Card>
            )}
                ...
                <Card className="border-blue-200 dark:border-blue-800 bg-blue-50/50 dark:bg-blue-950/20">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium text-blue-700 dark:text-blue-400">
                            {t('checkin.stats.successRate', 'Tỷ lệ thành công')}
                        </CardTitle>
                    ...
                    <CardContent>
                        ...
                        <Progress value={stats.success_rate} className="mt-2 h-2" />
                    </CardContent>
                </Card>
            )}
        </div>
