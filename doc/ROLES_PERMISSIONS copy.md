...
### 1.7 Loại vé (Ticket Types) - Public

| Chức năng | Endpoint | Method |
|-----------|----------|--------|
| Xem danh sách loại vé của event | `/events/:eventId/ticket-types` | GET |
| Xem chi tiết loại vé | `/events/:eventId/ticket-types/:id` | GET |

### 1.8 Upload (Cần auth)

| Chức năng | Endpoint | Method |
|-----------|----------|--------|
| Upload hình ảnh | `/upload/image` | POST |

### 1.9 Hoàn tiền (Refunds)
...
### 1.10 Trợ lý AI (Public - không cần auth)
...
| Chức năng | Endpoint | Method |
|-----------|----------|--------|
| Quét QR check-in | `/check-in/scan` | POST |
| Xem thống kê check-in cơ bản | `/check-in/events/:eventId/stats` | GET |
| Xem lịch sử check-in | `/check-in/events/:eventId/history` | GET |
...
| Cập nhật loại vé | `/events/:eventId/ticket-types/:id` | PATCH |
| Vô hiệu hóa loại vé | `/events/:eventId/ticket-types/:id/disable` | POST |
| Xóa loại vé | `/events/:eventId/ticket-types/:id` | DELETE |
...
| Xem chi tiết vé | `/events/:eventId/tickets/:id` | GET |
| Thu hồi vé | `/events/:eventId/tickets/:id/revoke` | POST |
| Tạo vé test (development) | `/events/:eventId/tickets/test/create` | POST |
...
| Chức năng | Endpoint | Method |
|-----------|----------|--------|
| Xem thống kê theo giờ | `/check-in/events/:eventId/stats/hourly` | GET |
| Xem thống kê theo cổng | `/check-in/events/:eventId/stats/gates` | GET |
| Xem hiệu suất nhân viên | `/check-in/events/:eventId/stats/staff` | GET |
| Xem scan logs | `/check-in/events/:eventId/logs` | GET |
| Xuất báo cáo check-in | `/check-in/events/:eventId/export` | GET |
...
| Xem danh sách tổ chức của mình | `/organizations/my-organizations` | GET |
...
| Chức năng | Endpoint | Method |
|-----------|----------|--------|
| Xem danh sách thành viên | `/organizations/:organizationId/members` | GET |
| Thêm thành viên mới | `/organizations/:organizationId/members` | POST |
| Cập nhật role thành viên | `/organizations/:organizationId/members/:memberId/role` | PUT |
| Xóa thành viên | `/organizations/:organizationId/members/:memberId` | DELETE |
...
> **Lưu ý:** EVENT_MANAGER cũng có thể xóa loại vé (đã được gộp vào mục 3.2)
...
| Bật/tắt payout cho tổ chức | `/organizations/:id/payout/status` | PUT |
...
| Bật/tắt danh mục | `/categories/:id/toggle-active` | PATCH |

> **Lưu ý:** Hiện tại không có API quản lý Users riêng biệt. User được quản lý gián tiếp thông qua Auth và Organization Members.
...
| Mature pending shares thủ công | `/payouts/mature-shares` | POST |
